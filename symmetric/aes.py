...
class AES:
    ...
    def key_expansion(self):
        ...
        words=[
            key_bytes[i:i+4] for i in range(0,16,4)
        ]
        for i in range(4,44):
            temp = words[i-1].copy()
            if i%4==0:
            ...
            new_word = [
                words[i-4][j] ^ temp[j] for j in  range(4)
            ]
        ...
        for i in range(0,44,4):
            ...
            self.round_keys.append(bytes(round_key))
    ...
    def mix_columns(self, state):
        for col in range(4):
            ...
            state[0][col] = (
                self.gmul(a0, 2) ^ self.gmul(a1, 3) ^ a2 ^ a3
            )

            state[1][col] = (
                a0 ^ self.gmul(a1, 2) ^ self.gmul(a2, 3) ^ a3
            )

            state[2][col] = (
                a0 ^  a1 ^ self.gmul(a2, 2) ^ self.gmul(a3, 3)
            )

            state[3][col] = (
                self.gmul(a0, 3) ^ a1 ^ a2 ^ self.gmul(a3, 2)
            )

        return state


    # Inverse Round Operations for dec
    ...
    def inv_sub_bytes(self, state):
        for row in range(4):
            for col in range(4):
                state[row][col] = self.INV_S_BOX[state[row][col]]
